...
def plot_paper(k, l_nl, 
    ...
    for i, l in enumerate(l_nl):
        ...
        ax3.plot(k, VP_nonlocal(k, l)[1], color=okabe_ito[i], lw=1.2,label=r'$l_c = {:.0f}\,\mathrm{{km}}$'.format(l/1e3))
    linestyles = ['-', '-.', '--']
    ...
    fig.savefig("dispersion_VP_nonlocal_a005.png", bbox_inches="tight")
...
def plot_presentation(k, l_nl, 
    ...
    ax2.plot(k, omega_vp_div, color="#777777", lw=1.5, alpha = 0.8,label='VP')
    for i, l in enumerate(l_nl):
        ax2.plot(k, VP_nonlocal(k, l)[0], color=okabe_ito[i], lw=1.2,label=r'$l_c = {:.0f}\,\mathrm{{km}}$'.format(l/1e3))
    ax2.set_xscale('log')
    ax2.set_yscale('log')
